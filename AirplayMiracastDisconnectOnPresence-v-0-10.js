...
/**
 * ShareAutoDisconnect v0.10
 *
 * Fixes:
 * 
 * - Keeps Wireless retry from looping after wireless connection has been confirmed
 * - Remove buttons from OSD display
 * - Remove peripheralID from Userinterface message prompt commands
 *
 * Behavior:
...
const SETTINGS = {
  ...
  // Options only show on Controller
  ...
  wirelessRetryIntervalSeconds: 30,  // retry every 30s until Activity/Transport exist
...
const STATE = {
  ...
  wirelessRetryStoppedLogged: false,
...
async function closePrompt(reason = 'unknown') {
  ...
  const OSDbase = { FeedbackId: PROMPT_FEEDBACK_ID };
  const Controllerbase = { FeedbackId: PROMPT_FEEDBACK_ID };

  if (SETTINGS.showPromptOnOSD) {
    await safePromptClear({ ...OSDbase, Target: 'OSD' });
  }
  ...
  if (SETTINGS.showPromptOnNavigator) {
    ...
    if (!cleared) {
      await safePromptClear({ ...Controllerbase, Target: 'Controller' });
    }
  }
...
async function showPrompt() {
  ...
  if (SETTINGS.showPromptOnOSD) {
    const ok = await safePromptDisplay({ ...OSDbase, Target: 'OSD' });
  ...
  if (SETTINGS.showPromptOnNavigator) {
    ...
    if (!shown) {
      shown = await safePromptDisplay({ ...Controllerbase, Target: 'Controller' });
      log('Prompt display Controller:', shown);
    }
...
function stopWirelessRetryLoop(reason = 'unknown') {
  // Only do anything if the loop is actually running
  const wasRunning = !!STATE.timers.wirelessRetryTimer;

  clearTimer('wirelessRetryTimer');

  // Log only once, and only if we actually stopped a running loop
  if (wasRunning && !STATE.wirelessRetryStoppedLogged) {
    log('Wireless late nodes now available; stopping retry loop', { reason });
    STATE.wirelessRetryStoppedLogged = true;
  }
}
